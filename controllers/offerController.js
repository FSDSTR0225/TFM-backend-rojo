  ...
  getCandidatesByOffer: async (req, res) => {
    try {
      ...
      const offer = await Offer.findById(offerId)
        .populate(
          "applicants.user",
          "name email surname avatar appliedDate role.developer.skills role.developer.location"
  ...
  updateCandidateStatus: async (req, res) => {
    try {
      ...
      await offer.populate(
        "applicants.user",
        "name surname email avatar developer role.developer.location role.developer.skills"
      );
